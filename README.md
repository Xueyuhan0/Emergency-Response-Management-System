...
_UI Interaction Function_:
remove_item method: Ensure removal of an emergency event from both the display list (self.queue_listbox) and the underlying data structure (self.queue.heap) is synchronized. Deleting directly from self.queue_listbox and attempting to remove from self.queue.heap by index can lead to synchronization issues. 
Instead:Remove the item from self.queue.heap.Update self.queue_listbox to reflect the updated list after 
removal.show_details method: Implement a tooltip feature for displaying detailed information when hovering over an emergency event in the list box. <br/>
